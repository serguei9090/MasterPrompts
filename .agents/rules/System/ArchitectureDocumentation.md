...
To maintain a "Single Source of Truth" and avoid repetitive search commands, all core architectural elements MUST be documented in `docs/Documentation/architecture/`.
...
- **New File**: Add to the relevant file in `docs/Documentation/architecture/layers/` and index in `gemini.md`.
...
- **New Feature/Flow**: Update the system diagram in `docs/Documentation/architecture/diagrams.md` or `gemini.md`.

## 2. Directory Structure (`docs/Documentation/architecture/`)
- **`gemini.md`**: Main entry point. Contains the high-level C4-style System Context diagram and links to `AGENTS.md` for persona definitions.
...
1. **Consult `docs/Documentation/architecture/gemini.md`**: Use it to find where a logic block *should* live.
2. **Follow the Personas**: Consult `AGENTS.md` to understand the "soul" of each specialized role before writing code.
